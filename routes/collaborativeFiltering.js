...
exports.getData = function () {
    ...
    if (data[0].kind == '말티즈') { var kind = 90; }
    else if (data[0].kind == '푸들') { var kind = 92; }
    else if (data[0].kind == '진돗개') { var kind = 50; }

    var age = data[0].age;

    if (data[0].gender == '암컷') { var gender = 1; }
    else { var gender = 10000; }

    if (data[0].neutralization == 'YES') { var neutralization = 1; }
    else { var neutralization = 100; }

    if (data[0].enrollment == 'YES') { var enrollment = 1; }
    else { var enrollment = 100; }

    basicDataSet.basicDataSet[user_name] = {
        '강아지/고양이': 1,
        '품종': kind,
        '나이': (age / 10),
        '성별': gender,
    ...
    // console.log(insuranceDataSet.insuranceDataSet);
